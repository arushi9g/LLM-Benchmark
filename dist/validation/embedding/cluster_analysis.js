...
async function runClustering() {
  ...
  try {
    ...
    // The call to the function remains the same.
    ...
    // The 'ans.clusters' array holds the cluster index for each embedding.
    // We can use this to group the original texts.

    // Create an array of arrays to hold the texts for each cluster
    const clusters: { text: string }[][] = Array.from({ length: numberOfClusters }, () => []);

    // Assign each text to its corresponding cluster
...
async function runClustering() {
    ...
    try {
        ...
        // Extract embeddings and corresponding prompts
